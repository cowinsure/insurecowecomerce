      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
              ...
              {product.images.map((image, index) => (
                <button
                  ...
                  className={`aspect-square rounded-lg overflow-hidden border-2 transition-colors ${
                    selectedImage === index ? "border-green-500" : "border-green-200 dark:border-green-700"
                  }`}
                >
                  <Image
          ...
          <div className="space-y-6">
            <div>
              <span className="text-green-600 dark:text-green-400 font-medium">{product.category}</span>
              <h1 className="text-3xl font-bold text-green-800 dark:text-green-200 mt-2">{product.name}</h1>
            ...
            <div className="flex items-center space-x-4">
              <span className="text-3xl font-bold text-green-800 dark:text-green-200">${product.price}</span>
            ...
            <p className="text-green-700 dark:text-green-300 leading-relaxed">{product.description}</p>

            {/* Quantity and Actions */}
            <div className="space-y-4">
              <div className="flex items-center space-x-4">
                <Label className="text-green-800 dark:text-green-200">Quantity:</Label>
                <div className="flex items-center border border-green-200 dark:border-green-700 rounded-lg">
                  ...
                  <span className="px-4 py-2 text-green-800 dark:text-green-200 font-semibold">{quantity}</span>
                  <Button
                    variant="ghost"
                    size="sm"
                ...
                {product.inStock && (
                  <span className="text-green-600 dark:text-green-400 text-sm">{product.stockCount} in stock</span>
                )}
              </div>
            ...
            <div className="bg-white dark:bg-green-900/50 rounded-xl p-6 border border-green-100 dark:border-green-800">
              <h3 className="text-lg font-semibold text-green-800 dark:text-green-200 mb-4">Key Features</h3>
        ...
        <div className="bg-white dark:bg-green-900/50 rounded-xl border border-green-100 dark:border-green-800">
          ...
          <div className="p-6">
            {activeTab === "description" && (
              <div className="space-y-4">
                <h3 className="text-xl font-semibold text-green-800 dark:text-green-200">Product Description</h3>
                <p className="text-green-700 dark:text-green-300 leading-relaxed">{product.description}</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                  <div>
                    <h4 className="font-semibold text-green-800 dark:text-green-200 mb-2">Specifications</h4>
                  ...
                  <div>
                    <h4 className="font-semibold text-green-800 dark:text-green-200 mb-2">What's in the Box</h4>
                    <ul className="space-y-1 text-green-700 dark:text-green-300">
            ...
            {activeTab === "reviews" && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-semibold text-green-800 dark:text-green-200">Customer Reviews</h3>
                ...
                <div className="bg-green-50 dark:bg-green-900/30 rounded-lg p-6 border border-green-200 dark:border-green-700">
                  <h4 className="font-semibold text-green-800 dark:text-green-200 mb-4">Write a Review</h4>
                  {!isLoggedIn ? (
                    ...
                    <form onSubmit={handleReviewSubmit} className="space-y-4">
                      <div>
                        <Label className="text-green-800 dark:text-green-200">Rating</Label>
                      ...
                      <div>
                        <Label htmlFor="comment" className="text-green-800 dark:text-green-200">
                          Your Review
                        </Label>
                        <Textarea
                          id="comment"
                ...
                <div className="space-y-4">
                  {product.reviews_data.map((review) => (
                    <div key={review.id} className="border border-green-200 dark:border-green-700 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-2">
                          <span className="font-semibold text-green-800 dark:text-green-200">{review.user}</span>
                        ...
                        <span className="text-sm text-green-600 dark:text-green-400">{review.date}</span>
                      </div>
                      <p className="text-green-700 dark:text-green-300">{review.comment}</p>
                    </div>
                  ))}
                </div>
              </div>
